use std::mem::swap;
use crate::ciphers::{lfsr, SymmetricCipher};
use crate::ciphers::Shift::{Left, Right};
...
const X0: usize = 0b0000_0001;
const X2: usize = 0b0000_0100;
const X3: usize = 0b0000_1000;
const X5: usize = 0b0010_0000;
const X6: usize = 0b0100_0000;
const X7: usize = 0b1000_0000;

pub enum Skinny {
    Skinny64 { r: Option<usize> },
    Skinny128 { r: Option<usize> },
}

impl Skinny {
    pub fn v64() -> Skinny {
        Skinny::Skinny64 { r: None }
    }

    pub fn v128() -> Skinny {
        Skinny::Skinny128 { r: None }
    }
    ...
    fn lfsr(&self, i: usize, value: u8) -> u8 {
        match self {
            Skinny::Skinny64 { .. } => Self::lfsr_64(i, value),
            Skinny::Skinny128 { .. } => Self::lfsr_128(i, value)
        }
    }

    fn lfsr_64(i: usize, value: u8) -> u8 {
        match i {
            2 => lfsr(4, X3 ^ X2, Left, value as usize) as u8,
            3 => lfsr(4, X0 ^ X3, Right, value as usize) as u8,
            _ => panic!("Invalid lfsr index")
        }
    }

    fn lfsr_128(i: usize, value: u8) -> u8 {
        match i {
            2 => lfsr(8, X7 ^ X5, Left, value as usize) as u8,
            3 => lfsr(8, X0 ^ X6, Right, value as usize) as u8,
            _ => panic!("Invalid lfsr index")
        }
    }
